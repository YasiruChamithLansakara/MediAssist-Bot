...
# ✅ NEW: remove near-duplicate SECTION BODIES across headings
def drop_duplicate_section_bodies(formatted: str, threshold: float = 0.90) -> str:
    ...
    for title, body in parsed:
        ...
        body_norm = _normalize_for_dupe(body)

        dup_idx = -1
        for i, (kt, kb) in enumerate(kept):
            if not kt:
                continue
            kb_norm = _normalize_for_dupe(kb)
            if not body_norm or not kb_norm:
                continue
            if SequenceMatcher(None, body_norm, kb_norm).ratio() >= threshold:
                dup_idx = i
                break
...
def clean_openfda_heading_blocks(text: str, max_chars: int = 1200) -> str:
    ...
    heading_re = re.compile(
        r"(?i)\b(" + "|".join(re.escape(h) for h in headings) + r")\b\s*[:]?"
    )
    ...
    # ✅ NEW: remove duplicate bodies across headings (Warnings vs Overdosage etc.)
...
def clean_fda_warnings_section_level(text: str, max_chars: int = 1200) -> str:
    ...
    # ✅ NEW: remove duplicate bodies across sections
    out = drop_duplicate_section_bodies(out, threshold=0.90)
