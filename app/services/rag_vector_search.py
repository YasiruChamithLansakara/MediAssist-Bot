    ...
    def _init(self) -> None:
        ...
        try:
            if EMBEDDING_PROVIDER == "openai":
                ...
                if api_key:
                    self.embedder = OpenAIEmbedding(api_key, EMBEDDING_MODEL)
                else:
                    logger.warning("No LLM_API_KEY for OpenAI embeddings, trying local...")
            ...
            if self.embedder and self.embedder.model or EMBEDDING_PROVIDER == "tfidf":
                ...
                self.enabled = True
                logger.info(
                    f"RAG initialized: provider={EMBEDDING_PROVIDER}, "
                    f"dimension={dim}, db={VECTOR_DB_PATH}"
                )
            else:
                logger.warning("No working embedding backend available")
        except Exception as e:
